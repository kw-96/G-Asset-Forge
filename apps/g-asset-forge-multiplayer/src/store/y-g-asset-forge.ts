import { EventEmitter, isEqual, pick, throttle } from '@g-asset-forge/common';
import {
...
export class GAssetForgeBinding {
  private doc: Y.Doc;
  ...
  constructor(
    ...
    this.doc = yMap.doc!;
    // data
    editor.doc.on('sceneChange', this.gAssetForgeObserve);
  ...
  // editor --> remote
  private gAssetForgeObserve = (ops: IChanges) => {
  ...
  private yMapObserve = (event: YMapEvent<any>) => {
    ...
    devLog('------ parse to g-asset-forge changes ------');
    devLog(changes);
  ...
  destroy() {
    ...
    this.editor.doc.off('sceneChange', this.gAssetForgeObserve);
