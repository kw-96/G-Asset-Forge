...
import { CursorArrowIcon, HandIcon, SquareIcon, CircleIcon, TextIcon, FrameIcon, Pencil1Icon, ImageIcon, StarIcon, TriangleRightIcon, CropIcon } from '@radix-ui/react-icons';
...
const FloatingToolbar = ({ className }) => {
    const { activeTool, switchTool } = useToolSwitch();
    const handleToolClick = (toolId) => {
        switchTool(toolId);
    };
    return (_jsx(FloatingToolbarContainer, { className: className, children: toolGroups.map((group, groupIndex) => (_jsxs(React.Fragment, { children: [group.tools.map((tool) => (_jsx(Tooltip, { content: `${tool.label} (${tool.shortcut})`, children: _jsx(ToolButton, { icon: _jsx(tool.icon, {}), onClick: () => handleToolClick(tool.id), "$active": activeTool === tool.id, variant: "ghost", size: "sm", "aria-label": tool.label }) }, tool.id))), groupIndex < toolGroups.length - 1 && _jsx(ToolSeparator, {})] }, group.name))) }));
};
