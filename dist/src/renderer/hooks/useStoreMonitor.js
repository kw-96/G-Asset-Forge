...
export function useStoreMonitor(options = {}) {
    const { enableRenderTracking = true, enableStateChangeLogging = false, renderThreshold = 20, stateChangeThreshold = 50, monitorInterval = 5000, // 5秒
     } = options;
    ...
    useEffect(() => {
        ...
        // 记录渲染日志
        reactLoopFixToolkit.debugLogger.logRender('AppStore', renderCountRef.current, undefined, 'store state change');
        // 检查是否渲染过于频繁
        if (renderCountRef.current > renderThreshold) {
            reactLoopFixToolkit.debugLogger.warn('store-monitor', `AppStore渲染次数过多: ${renderCountRef.current}`, { threshold: renderThreshold, interval }, 'useStoreMonitor');
        }
    });
    // 监控状态变化
    useEffect(() => {
        ...
        if (hasStateChanged) {
            ...
            if (enableStateChangeLogging) {
                ...
                Object.keys(currentState).forEach(key => {
                    if (currentState[key] !== lastStateRef.current[key]) {
                        changedFields.push(key);
                    }
                });
                reactLoopFixToolkit.debugLogger.debug('store-monitor', 'AppStore状态变化', {
                    changedFields,
            ...
            if (stateChangeCountRef.current > stateChangeThreshold) {
                reactLoopFixToolkit.debugLogger.warn('store-monitor', `AppStore状态变化过于频繁: ${stateChangeCountRef.current}`, { threshold: stateChangeThreshold }, 'useStoreMonitor');
            }
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            const averageRenderInterval = renderTimesRef.current.length > 0
                ...
                : 0;
            const suspiciousActivity = renderCountRef.current > renderThreshold ||
            ...
            if (suspiciousActivity) {
                reactLoopFixToolkit.debugLogger.warn('store-monitor', 'AppStore检测到可疑活动', newStats, 'useStoreMonitor');
                // 检查是否存在无限循环
                const hasInfiniteLoop = reactLoopFixToolkit.detectInfiniteLoop();
                if (hasInfiniteLoop) {
                    reactLoopFixToolkit.debugLogger.error('store-monitor', 'AppStore可能存在无限循环', { stats: newStats }, 'useStoreMonitor');
                }
            }
        }, monitorInterval);
    ...
    const resetStats = () => {
        ...
        reactLoopFixToolkit.debugLogger.info('store-monitor', 'AppStore监控统计已重置', {}, 'useStoreMonitor');
    };
    // 生成监控报告
    ...
    // 检查特定状态字段的变化频率
    const checkFieldChangeFrequency = (fieldName, timeWindow = 1000) => {
        // 这里可以实现更详细的字段级监控
        // 暂时返回基本信息
        return {
            fieldName,
...
//# sourceMappingURL=useStoreMonitor.js.map