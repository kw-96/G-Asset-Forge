...
(function (MemoryEvent) {
    MemoryEvent["MEMORY_WARNING"] = "memory:warning";
    MemoryEvent["MEMORY_CRITICAL"] = "memory:critical";
    MemoryEvent["GARBAGE_COLLECTED"] = "memory:gc";
    MemoryEvent["OBJECT_POOLED"] = "memory:pooled";
    MemoryEvent["CACHE_CLEARED"] = "memory:cache-cleared";
...
export class MemoryManager extends EventEmitter {
    ...
    updateMemoryStats() {
        ...
        if ('memory' in performance && performance.memory) {
            this.stats.totalMemory = performance.memory.usedJSHeapSize / (1024 * 1024);
        }
    ...
    handleObjectRemoval(obj) {
        ...
        if (obj.type === 'image') {
            const img = obj;
            const src = img.getSrc ? img.getSrc() : undefined;
            if (src) {
