...
(function (ErrorCategory) {
    ErrorCategory["REACT_ERROR"] = "react_error";
...
(function (ErrorPattern) {
    ...
    ErrorPattern["MEMORY_LEAK"] = "memory_leak";
    ErrorPattern["RENDER_THRASHING"] = "render_thrashing";
    ErrorPattern["STATE_MUTATION"] = "state_mutation";
    ErrorPattern["ASYNC_RACE_CONDITION"] = "async_race_condition";
    ErrorPattern["COMPONENT_LIFECYCLE"] = "component_lifecycle";
    ErrorPattern["HOOK_DEPENDENCY"] = "hook_dependency";
    ErrorPattern["CONTEXT_PROVIDER"] = "context_provider";
...
export class ErrorAnalyzer {
    constructor() {
        ...
        // 组件名称提取正则
        this.componentNameRegex = /at\s+(\w+)\s+\(/g;
    }
    static getInstance() {
    ...
    analyzeError(error, errorInfo) {
        ...
        // 记录分析结果
        reactLoopFixToolkit.debugLogger.info('error-analyzer', '错误分析完成', {
            category,
            pattern,
            confidence,
     ...
     * 检测错误模式
     */
    detectErrorPattern(message, stack, componentStack) {
     ...
     * 检测无限循环模式
     */
    detectInfiniteLoopPattern(_message, stack, componentStack) {
        // 检查是否有重复的函数调用
     ...
     * 计算置信度
     */
    calculateConfidence(pattern, message, stack) {
        if (!pattern)
            return 0.3;
        let confidence = 0.5;
     ...
     * 生成错误描述
     */
    generateDescription(pattern, category) {
        if (pattern) {
            switch (pattern) {
                case ErrorPattern.INFINITE_LOOP:
     ...
     * 生成修复建议
     */
    generateSuggestions(pattern, _category, components) {
        const suggestions = [];
        if (pattern) {
            switch (pattern) {
                case ErrorPattern.INFINITE_LOOP:
                    suggestions.push('检查useEffect的依赖数组，确保不包含每次渲染都变化的值', '使用useCallback和useMemo稳定化函数和对象引用', '避免在渲染过程中直接调用setState', '检查是否存在组件间的循环依赖');
                    break;
                case ErrorPattern.HOOK_DEPENDENCY:
                    suggestions.push('检查Hook的依赖数组是否包含所有使用的变量', '使用ESLint的react-hooks/exhaustive-deps规则', '考虑使用useCallback稳定化函数引用', '避免在依赖数组中使用对象或数组字面量');
                    break;
                case ErrorPattern.MEMORY_LEAK:
                    suggestions.push('在useEffect的清理函数中移除事件监听器', '清理定时器和间隔器', '取消未完成的网络请求', '检查是否有循环引用导致的内存泄漏');
                    break;
                case ErrorPattern.STATE_MUTATION:
                    suggestions.push('使用setState或状态更新函数修改状态', '避免直接修改state对象或数组', '使用扩展运算符或Object.assign创建新对象', '考虑使用Immer库处理复杂的状态更新');
                    break;
            }
        }
        // 基于组件的建议
        ...
        // 通用建议
        suggestions.push('查看浏览器控制台的完整错误信息', '使用React DevTools检查组件状态和props', '考虑添加错误边界来捕获和处理错误');
        return suggestions;
    }
    /**
     * 评估恢复能力
     */
    assessRecoverability(pattern, _category) {
        if (pattern) {
            switch (pattern) {
                case ErrorPattern.INFINITE_LOOP:
