**G-Asset Forge 图形编辑器**，一款专为游戏资产创建和图形设计打造的基于 Canvas 实现的专业图形编辑器。

简体中文 | [English](./README.md)

[体验网址](https://blog.fstars.wang/app/g-asset-forge/)
...
[中文文档](https://f-star.github.io/g-asset-forge-document/)
...
构建产物在目录：`apps/g-asset-forge/build`.
...
[地址](https://www.figma.com/community/file/1224385128783567603/g-asset-forge-icons)

## 多人协同

多人协同基于 yjs 实现，并使用了 hocuspocus 库（对 yjs 的进一步封装）。

- `@g-asset-forge/g-asset-forge-multiplayer`：协同的编辑器前端实现逻辑，该包拷贝了 `@g-asset-forge/g-asset-forge`（纯客户端），在其基础上加入了多人协同逻辑，比如多人光标，并调用了一些接口，如获取图纸名、当前用户信息。
- `@g-asset-forge/workbench`：工作台项目，对图纸进行管理，以支持登录注册、创建图纸、打开图纸、删除图纸、授权等操作。
- `@g-asset-forge/backend`：如果你要做多人协同，需要 **实现后端 Restful 接口服务**，这里该包用到的后端接口，并提供 **基于 hocuspocus 的 websocket 服务**。
...
```sh
pnpm run -F @g-asset-forge/backend run migrate:dev
```
