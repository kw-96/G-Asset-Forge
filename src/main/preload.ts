...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  config: {
    save: (configKey: string, configData: unknown) => safeInvoke('config:save', configKey, configData),
    load: (configKey: string) => safeInvoke('config:load', configKey),
    exists: (configKey: string) => safeInvoke('config:exists', configKey),
    delete: (configKey: string) => safeInvoke('config:delete', configKey),
    list: () => safeInvoke('config:list')
  },
  ...
  window: {
    ...
    setMinimumSize: (width: number, height: number) => safeInvoke('window:setMinimumSize', width, height),
    removeMinimumSize: () => safeInvoke('window:removeMinimumSize'),
    getMinimumSize: () => safeInvoke('window:getMinimumSize'),
...
export interface ElectronAPI {
  ...
  config: {
    ...
    load: (configKey: string) => Promise<{ success: boolean; data?: any; path?: string; size?: number; lastModified?: number; error?: string }>;
    exists: (configKey: string) => Promise<{ success: boolean; data?: boolean; error?: string }>;
    delete: (configKey: string) => Promise<{ success: boolean; path?: string; error?: string }>;
    list: () => Promise<{ success: boolean; configs?: Array<{ key: string; filename: string; path: string; size: number; created: number; modified: number }>; error?: string }>;
  };
  window: {
    ...
    setMinimumSize: (width: number, height: number) => Promise<{ success: boolean; error?: string }>;
    removeMinimumSize: () => Promise<{ success: boolean; error?: string }>;
    getMinimumSize: () => Promise<{ success: boolean; data?: { width: number; height: number }; error?: string }>;
