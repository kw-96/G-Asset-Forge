...
import { H5Editor, type H5EditorOptions, type H5Page, type H5Component, type H5Template } from '../core/h5-editor';
import { H5EditorManager, type H5EditorManagerOptions, type H5Project } from '../core/h5-editor-manager';
import { type BackgroundSettings, type BackgroundPreset, BackgroundGradient } from '../background/BackgroundManager';
import { ImageExportProgress, type ImageExportOptions, type ImageExportResult } from '../export/ImageExportEngine';

// 页面信息类型（与 H5EditorManager.getAllPagesInfo 返回值保持一致）
export type H5PageInfo = {
  id: string;
  name: string;
  width: number;
  height: number;
  ...
  isCurrent: boolean;
  background: H5Page['background'];
  thumbnail?: string;
};

export interface H5EditorCanvasProps {
  width: number;
  height: number;
  mode?: 'mobile' | 'desktop';
  ...
  managerOptions?: H5EditorManagerOptions;
  
  // 事件回调
  onReady?: (editor: H5Editor, manager: H5EditorManager) => void;
  onPageChange?: (page: H5Page) => void;
  onComponentAdd?: (component: H5Component) => void;
  onComponentUpdate?: (component: H5Component) => void;
  onComponentRemove?: (componentId: string) => void;
  onBackgroundChange?: (background: H5Page['background']) => void;
  onCanvasSizeChange?: (size: { width: number; height: number }) => void;
  onTemplateLoad?: (template: H5Template) => void;
  onExportStart?: () => void;
  ...
  onProjectLoad?: (project: H5Project) => void;
  onProjectSave?: (project: H5Project) => void;
...
export interface H5EditorCanvasRef {
  // 编辑器访问
  ...
  // 页面管理
  createPage: (name: string, options?: Partial<H5Page>) => H5Page | null;
  ...
  // 背景设置
  setPageBackground: (background: H5Page['background']) => void;
  setCanvasSize: (width: number, height: number) => void;
  
  // 模板系统
  loadTemplate: (templateId: string) => boolean;
  saveAsTemplate: (name: string, category?: string) => H5Template | null;
  getTemplates: () => H5Template[];
  
  // 项目管理
  createNewProject: (name: string, options?: Partial<H5Project>) => H5Project | null;
  loadProject: (projectData: H5Project) => Promise<void>;
  saveCurrentProject: () => Promise<H5Project | null>;
  ...
  getFinalEffectPreview: (options: Partial<ImageExportOptions> & { format?: 'png' | 'jpg' }) => string;
  setPreviewUpdateCallback: (callback: (previewUrl: string) => void) => void;

  // 增强页面管理
  createPageWithTemplate: (name: string, templateId?: string, options?: Partial<H5Page>) => H5Page | null;
  duplicateCurrentPage: (newName?: string) => H5Page | null;
  deletePageById: (pageId: string) => boolean;
  getAllPagesInfo: () => Array<H5PageInfo>;
  reorderPages: (pageIds: string[]) => boolean;

  // 背景设置功能
  setColorBackground: (color: string) => void;
  setGradientBackground: (gradient: BackgroundGradient) => void;
  setImageBackground: (imageUrl: string, options?: Partial<ImageExportOptions>) => void;
  uploadImageBackground: (file: File) => Promise<string>;
  applyBackgroundPreset: (presetId: string) => boolean;
  createCustomBackgroundPreset: (name: string, category?: string, description?: string, tags?: string[]) => BackgroundPreset | null;
  getAllBackgroundPresets: () => BackgroundPreset[];
  getBackgroundPresetsByCategory: (category: string) => BackgroundPreset[];
  searchBackgroundPresets: (query: string) => BackgroundPreset[];
  getBackgroundCategories: () => string[];
  getCurrentBackground: () => BackgroundSettings | null;
  getBackgroundPreviewUrl: () => string;
  setBackgroundPreviewCallback: (callback: (previewUrl: string) => void) => void;

  // 图片导出功能
  exportImage: (options?: Partial<ImageExportOptions>) => Promise<ImageExportResult>;
  exportImageAsFile: (filename: string, options?: Partial<ImageExportOptions>) => Promise<File | null>;
  exportMultipleFormats: (formats: Array<{ format: 'png' | 'jpg' | 'webp'; quality?: number; scale?: number }>) => Promise<ImageExportResult[]>;
  getExportRecommendedSettings: (purpose: 'web' | 'print' | 'social' | 'icon') => Partial<ImageExportOptions>;
  ...
  setImageExportProgressCallback: (callback: (progress: ImageExportProgress) => void) => void;
  setImageExportCompleteCallback: (callback: (result: ImageExportResult) => void) => void;
...
export const H5EditorCanvas = forwardRef<H5EditorCanvasRef, H5EditorCanvasProps>(
  ({ 
    width, 
    height, 
    mode = 'mobile',
    ...
    useImperativeHandle(ref, () => ({
      ...
      // 页面管理
      createPage: (name: string, options?: Partial<H5Page>) => {
      ...
      // 组件管理
      addComponent: (component: H5Component) => {
      ...
      updateComponent: (componentId: string, updates: Partial<H5Component>) => {
      ...
      // 背景设置
      setPageBackground: (background: H5Page['background']) => {
      ...
      // 项目管理
      createNewProject: (name: string, options?: Partial<H5Project>) => {
        return managerRef.current?.createNewProject(name, options) || null;
      },
      loadProject: async (projectData: H5Project) => {
        if (managerRef.current) {
      ...
      getFinalEffectPreview: (options: Partial<ImageExportOptions> & { format?: 'png' | 'jpg' }) => {
        return managerRef.current?.getFinalEffectPreview(options as any) || '';
      },
      ...
      // 增强页面管理
      createPageWithTemplate: (name: string, templateId?: string, options?: Partial<H5Page>) => {
      ...
      // 图片导出功能
      exportImage: async (options?: Partial<ImageExportOptions>) => {
        if (!managerRef.current) {
          throw new Error('Manager not initialized');
        }
        return await managerRef.current.exportImage(options);
      },
      exportImageAsFile: async (filename: string, options?: Partial<ImageExportOptions>) => {
        if (!managerRef.current) {
          throw new Error('Manager not initialized');
        }
      ...
      setImageExportCompleteCallback: (callback: (result: ImageExportResult) => void) => {
