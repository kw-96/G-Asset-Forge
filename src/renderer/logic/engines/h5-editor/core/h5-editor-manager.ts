...
import { H5Editor, type H5EditorOptions, type H5Page } from './h5-editor';
import { EventEmitter } from '../utils/event-emitter';
import { BackgroundManager, type BackgroundSettings, type BackgroundPreset } from '../background/BackgroundManager';
import { ImageExportEngine, type ImageExportOptions, type ImageExportResult } from '../export/ImageExportEngine';

export interface H5EditorManagerOptions {
...
export interface H5EditorManagerEvents extends Record<string, (...args: any[]) => void> {
  modeChange(mode: 'h5' | 'suika'): void;
  editorReady(editor: H5Editor): void;
  editorDestroyed(): void;
  projectLoad(project: H5Project): void;
  projectSave(project: H5Project): void;
...
export class H5EditorManager {
  ...
  private emitter = new EventEmitter<H5EditorManagerEvents>();
  private options: H5EditorManagerOptions;
  private currentMode: 'h5' | 'suika' = 'h5';
  private currentProject: H5Project | null = null;
  private undoStack: H5Page[] = [];
  private redoStack: H5Page[] = [];
  ...
  constructor(options: H5EditorManagerOptions = {}) {
    this.options = {
   ...
   * 初始化H5编辑器
   */
  async initializeH5Editor(_container: HTMLElement, editorOptions: H5EditorOptions): Promise<H5Editor> {
    try {
      if (this.editor) {
   ...
   * 切换回H5模式
   */
  async switchToH5Mode(container: HTMLElement, editorOptions: H5EditorOptions): Promise<H5Editor> {
    try {
   ...
   * 进入H5编辑器模式
   */
  async enterH5Mode(container: HTMLElement, editorOptions: H5EditorOptions): Promise<H5Editor> {
   ...
   * 项目管理
   */
  createNewProject(name: string, options?: Partial<H5Project>): H5Project {
    const project: H5Project = {
  ...
  async loadProject(projectData: H5Project): Promise<void> {
    try {
      this.currentProject = {
        ...projectData,
        metadata: {
  ...
  async saveCurrentProject(): Promise<H5Project | null> {
  ...
  private restoreEditorState(state: H5Page): void {
    ...
    // 恢复页面
    const newPage = this.editor!.createPage(state.name, state);
    this.editor!.setCurrentPage(newPage.id);
  }
   ...
   * 生成页面缩略图
   */
  private generatePageThumbnail(_page: H5Page): string {
    try {
  ...
  createCustomBackgroundPreset(name: string, category: string = '自定义', description: string = '', tags: string[] = []): BackgroundPreset | null {
    if (!this.backgroundManager) {
      throw new Error('背景管理器未初始化');
    }
   ...
   * 图片导出功能
   */
  async exportImage(options?: Partial<ImageExportOptions>): Promise<ImageExportResult> {
  ...
  async exportImageAsFile(filename: string, options?: Partial<ImageExportOptions>): Promise<File | null> {
    if (!this.editor || !this.imageExportEngine) {
      throw new Error('编辑器或导出引擎未初始化');
    }
  ...
  getCurrentProject(): H5Project | null {
    return this.currentProject;
  }
   ...
   * 事件管理
   */
  on<T extends keyof H5EditorManagerEvents>(eventName: T, listener: H5EditorManagerEvents[T]): void {
    this.emitter.on(eventName, listener);
  }

  off<T extends keyof H5EditorManagerEvents>(eventName: T, listener: H5EditorManagerEvents[T]): void {
