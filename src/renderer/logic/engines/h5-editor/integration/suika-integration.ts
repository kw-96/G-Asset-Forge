...
import { H5Editor, type H5Component } from '../core/h5-editor';
import { H5EditorManager } from '../core/h5-editor-manager';
// 注意：本文件使用本地定义的 SuikaEditor 接口进行类型约束，
// 实际运行时的 SuikaEditor 实例由外部传入，无需在此处导入具体实现

// 假设的Suika接口（实际应该从Suika引擎导入）
interface SuikaEditor {
...
export interface SuikaH5IntegrationOptions {
...
export class SuikaH5Integration {
  ...
  private options: SuikaH5IntegrationOptions;

  constructor(options: SuikaH5IntegrationOptions = {}) {
    this.options = {
   ...
   * 初始化集成
   */
  initialize(h5Editor: H5Editor, suikaEditor: SuikaEditor, h5Manager?: H5EditorManager): void {
    if (this.isDestroyed) {
  ...
  private createSuikaObject(h5Component: H5Component): void {
  ...
  private updateSuikaObject(h5Component: H5Component, mapping: ObjectMapping): void {
    if (!this.suikaEditor) return;
  ...
  private updateH5Component(suikaObject: any, mapping: ObjectMapping): void {
    if (!this.h5Editor) return;
  ...
  private cleanupDeletedObjects(currentComponents: H5Component[]): void {
  ...
  private convertH5ComponentToSuikaObject(component: H5Component): any {
    // 将H5组件转换为Suika对象格式
    return {
  ...
  private convertH5ComponentToSuikaUpdates(component: H5Component): any {
    return {
      x: component.x,
      y: component.y,
      width: component.width,
  ...
  private convertSuikaObjectToH5Component(suikaObject: any): H5Component {
    // 将Suika对象转换为H5组件格式
    return {
  ...
  private convertSuikaObjectToH5Updates(suikaObject: any): Partial<H5Component> {
    return {
      x: suikaObject.x,
      y: suikaObject.y,
      width: suikaObject.width,
