...
export type BackgroundSettings = BackgroundColor | BackgroundGradient | BackgroundImage;
...
export interface BackgroundManagerEvents extends Record<string, (...args: any[]) => void> {
  backgroundChange(background: BackgroundSettings): void;
  previewUpdate(previewUrl: string): void;
  presetLoad(preset: BackgroundPreset): void;
...
export class BackgroundManager {
  private emitter = new EventEmitter<BackgroundManagerEvents>();
  private presets: Map<string, BackgroundPreset> = new Map();
  private currentBackground: BackgroundSettings | null = null;
  ...
  private initializePresets(): void {
    const defaultPresets: BackgroundPreset[] = [
      // 纯色预设
      {
        id: 'color-white',
        name: '纯白色',
   ...
   * 设置渐变背景
   */
  setGradientBackground(gradient: Omit<BackgroundGradient, 'type'>): void {
   ...
   * 设置图片背景
   */
  setImageBackground(imageUrl: string, options?: Partial<Omit<BackgroundImage, 'type' | 'url'>>): void {
   ...
   * 创建自定义预设
   */
  createCustomPreset(name: string, category: string = '自定义', description: string = '', tags: string[] = []): BackgroundPreset | null {
   ...
   * 按分类获取预设
   */
  getPresetsByCategory(category: string): BackgroundPreset[] {
   ...
   * 搜索预设
   */
  searchPresets(query: string): BackgroundPreset[] {
   ...
   * 事件管理
   */
  on<T extends keyof BackgroundManagerEvents>(eventName: T, listener: BackgroundManagerEvents[T]): void {
    this.emitter.on(eventName, listener);
  }

  off<T extends keyof BackgroundManagerEvents>(eventName: T, listener: BackgroundManagerEvents[T]): void {
