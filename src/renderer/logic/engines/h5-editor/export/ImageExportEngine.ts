...
import { SharpImageProcessor, type SharpProcessingOptions } from '../utils/sharp-integration';
import { H5Component, H5Page } from '../types';
...
export interface ImageExportEngineEvents extends Record<string, (...args: any[]) => void> {
  exportStart(options: ImageExportOptions): void;
  exportProgress(progress: ImageExportProgress): void;
  exportComplete(result: ImageExportResult): void;
...
export class ImageExportEngine {
  private emitter = new EventEmitter<ImageExportEngineEvents>();
  ...
  async exportImage(
    ...
    try {
      ...
      const errorResult: ImageExportResult = {
        success: false,
        size: 0,
  ...
  private async exportMultipleFormatsWithSharp(
    ...
    try {
      this.emitter.emit('exportStart', { format: 'png', quality: 1, scale: 1 } as ImageExportOptions);
      this.emitProgress('preparing', 0, '准备批量导出...');
      
      // 准备渲染画布
      const dimensions = this.calculateDimensions(page, { scale: 1 } as ImageExportOptions);
      ...
      // 转换Sharp结果为导出结果
      const results: ImageExportResult[] = sharpResults.map((sharpResult, index) => {
      ...
      const errorResults: ImageExportResult[] = formats.map(formatConfig => ({
        success: false,
        size: 0,
        format: formatConfig.format,
        dimensions: { width: 0, height: 0 },
   ...
   * 验证导出选项
   */
  private validateOptions(options: ImageExportOptions): void {
   ...
   * 计算最终尺寸
   */
  private calculateDimensions(page: H5Page, options: ImageExportOptions): { width: number; height: number } {
   ...
   * 计算内容边界
   */
  private calculateContentBounds(page: H5Page): { x: number; y: number; width: number; height: number } | null {
    ...
    page.components.forEach((component: H5Component) => {
   ...
   * 发送进度事件
   */
  private emitProgress(stage: ImageExportProgress['stage'], progress: number, message: string): void {
   ...
   * 获取推荐的导出设置
   */
  getRecommendedSettings(purpose: 'web' | 'print' | 'social' | 'icon'): Partial<ImageExportOptions> {
   ...
   * 事件管理
   */
  on<T extends keyof ImageExportEngineEvents>(eventName: T, listener: ImageExportEngineEvents[T]): void {
    this.emitter.on(eventName, listener);
  }

  off<T extends keyof ImageExportEngineEvents>(eventName: T, listener: ImageExportEngineEvents[T]): void {
