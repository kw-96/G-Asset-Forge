...
export class SharpImageProcessor {
  ...
  static async processBuffer(
    ...
    _height: number,
    options: SharpProcessingOptions
  ): Promise<Omit<SharpProcessingResult, 'processingTime'>> {
    try {
  ...
  static async processMultipleFormats(
    canvas: HTMLCanvasElement,
    formats: Array<{ format: 'png' | 'jpg' | 'webp'; options?: Partial<SharpProcessingOptions> }>
  ): Promise<SharpProcessingResult[]> {
    const results: SharpProcessingResult[] = [];
    ...
    for (const formatConfig of formats) {
      const options: SharpProcessingOptions = {
