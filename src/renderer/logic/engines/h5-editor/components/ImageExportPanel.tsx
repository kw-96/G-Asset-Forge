...
import { ImageExportEngine, type ImageExportOptions, type ImageExportResult, type ImageExportProgress } from '../export/ImageExportEngine';
import { ExportOptimizer, type ExportOptimizationSuggestion, type ExportPerformanceMetrics } from '../export/ExportOptimizer';
...
interface IImageExportPanelProps {
  ...
  onExportComplete?: (result: ImageExportResult) => void;
...
export const ImageExportPanel: React.FC<IImageExportPanelProps> = ({
  ...
  const [exportProgress, setExportProgress] = useState<ImageExportProgress | null>(null);
  const [exportResult, setExportResult] = useState<ImageExportResult | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string>('');
  
  // 优化相关状态
  const [suggestions, setSuggestions] = useState<ExportOptimizationSuggestion[]>([]);
  ...
  const applyPresetSettings = useCallback((purpose: 'web' | 'print' | 'social' | 'icon') => {
    ...
    if (sourceCanvas && optimizerRef.current) {
      const currentOptions: ImageExportOptions = {
        format,
        quality: quality / 100,
        scale,
  ...
  // 应用优化建议
  const handleApplySuggestion = useCallback((suggestion: ExportOptimizationSuggestion) => {
