import { cloneDeep, isEqual, noop } from '../../../common';
import {
  type IMatrixArr,
  invertMatrix,
  type IPoint,
  ...
  onDrag(e: PointerEvent) {
    ...
    const isOneDirResizeWithoutRotateHandle =
      ...
      (this.editor.selectedElements.size() > 1 ||
        (this.editor.selectedElements.getItems()[0]?.getRotate() ?? 0) % HALF_PI === 0);
    ...
    if (isTwoDirResizeHandle || isOneDirResizeWithoutRotateHandle) {
      ...
      this.lastDragPoint = {
        x: (this.lastDragPoint?.x ?? 0) + objectSnapOffset.x,
        y: (this.lastDragPoint?.y ?? 0) + objectSnapOffset.y,
      };
    }
