import { cloneDeep, parseHexToRGBA, parseRGBAStr } from '../../../common';
import {
  commandsToStr,
...
export class SuikaPath extends SuikaGraphics<PathAttrs> {
    ...
    const rect =
      this.attrs.height === 0
        ? resizeLine(type, newPos, oldRect, {
            keepPolarSnap: keepRatio ?? false,
            scaleFromCenter: scaleFromCenter ?? false,
          })
        : resizeRect(type, newPos, oldRect, {
            keepRatio: keepRatio ?? false,
            scaleFromCenter: scaleFromCenter ?? false,
            flip: flipWhenResize ?? false,
          });
  ...
  private _realDraw(
    ...
    ctx.save();
    ctx.transform(...(transform as [number, number, number, number, number, number]));
  ...
  getClosestAnchor(params: { point: IPoint; tol: number }) {
    ...
    for (let pathIdx = 0; pathIdx < pathData.length; pathIdx++) {
      ...
      for (let i = (pathItem.segs?.length ?? 0) - 1; i >= 0; i--) {
  ...
  setSeg(pathIdx: number, segIdx: number, partialSeg: Partial<ISegment>) {
    ...
    if (!pathItem.segs) {
      throw new Error(`pathItem.segs is undefined`);
    }
  ...
  override getSVGTagHead(offset?: IPoint) {
    ...
    if (offset) {
      const offsetPoint = offset as any;
      tf[4] += offsetPoint.x;
      tf[5] += offsetPoint.y;
    }
  ...
  override getLayerIconPath(): string {
    ...
    const matrix = new Matrix()
      .prepend(new Matrix(...(this.getWorldTransform() as [number, number, number, number, number, number])))
    ...
    commands.forEach((cmd: any) => {
      cmd.points.forEach((pt: any, idx: number) => {
