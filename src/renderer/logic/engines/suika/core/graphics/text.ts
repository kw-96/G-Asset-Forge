import { escapeHtml, parseRGBAStr } from '../../common';
import {
  applyInverseMatrix,
  calcGlyphInfos,
  calcTextSize,
...
export class SuikaText extends SuikaGraphics<TextAttrs> {
  ...
  protected override getSVGTagHead(offset?: IPoint) {
    ...
    tf[5] += ((this.attrs as any).fontSize ?? 0);
    if (offset) {
  ...
  getCursorIndex(point: IPoint) {
    ...
    while (left <= right) {
      ...
      if (point.x < (glyph as any).position.x) {
        right = mid - 1;
      } else {
        left = mid + 1;
      }
    ...
    if (
      (glyphs[left] as any).position.x - point.x >
      point.x - (glyphs[right] as any).position.x
    ) {
      return right;
    }
    return left;
