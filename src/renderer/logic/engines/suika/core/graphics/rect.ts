...
export class SuikaRect extends SuikaGraphics<RectAttrs> {
  ...
  override getControlHandles(zoom: number, initial?: boolean) {
    ...
    for (let i = 0; i < infos.length; i++) {
      const info = infos[i];
      let cornerRadius = info?.cornerRadius ?? 0;
      ...
      let x = (info?.origin.x ?? 0) + (info?.direction.x ?? 0) * cornerRadius;
      let y = (info?.origin.y ?? 0) + (info?.direction.y ?? 0) * cornerRadius;
