...
export class SuikaGraphics<ATTRS extends GraphicsAttrs = GraphicsAttrs> {
  ...
  hitTestChildren(point: IPoint, padding = 0): boolean {
    ...
    for (let i = children.length - 1; i >= 0; i--) {
      ...
      if (!child) continue;
      if (child.hitTest(point, padding)) {
        return true;
      }
    }
    return false;
  ...
  updateByControlHandle(
    ...
    this.updateAttrs(rect as Partial<GraphicsAttrs> & IAdvancedAttrs, { finishRecomputed: true });
  }
  ...
  draw(drawInfo: IDrawInfo) {
    ...
    ctx.save();
    ctx.transform(...(this.attrs.transform as [number, number, number, number, number, number]));
