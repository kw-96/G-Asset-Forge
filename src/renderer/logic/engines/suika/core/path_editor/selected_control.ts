import { cloneDeep, parseHexToRGBA, throttle } from '../../common';
import { getRotatedRectByTwoPoint, IPoint, isPointEqual } from '../../geo';
...
export class SelectedControl {
  ...
  private getItemsNeedDraw() {
    ...
    for (const control of controls) {
      ...
      const segCount = pathItem?.segs?.length ?? 0;
      const closed = pathItem?.closed ?? false;
  ...
  public generateControls(): ControlHandle[] {
    ...
    for (let i = 0; i < pathData.length; i++) {
      ...
      for (let j = 0; j < (pathItem?.segs?.length ?? 0); j++) {
        ...
        for (let handleIdx = 0; handleIdx < handles.length; handleIdx++) {
          ...
          if (isPointEqual(handle as IPoint, anchor)) {
            continue;
          }
          ...
          const rect = getRotatedRectByTwoPoint(anchor, handle as IPoint);
