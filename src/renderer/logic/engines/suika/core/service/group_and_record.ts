...
export const groupAndRecord = (
  ...
  const lastGraphics = graphicsArr.at(-1) as SuikaGraphics;
  const parentOfGroup = lastGraphics.getParent() as SuikaGraphics;
  const parentOfGroupInvertTf = invertMatrix(parentOfGroup.getWorldTransform() as IMatrixArr);
  ...
  for (const graphics of graphicsArr) {
    transaction.recordOld(graphics.attrs.id ?? '', {
      parentIndex: cloneDeep(graphics.attrs.parentIndex ?? { guid: '', position: '' }),
