...
export const ungroupAndRecord = (
  ...
  for (const [parentId, selectedChildSet] of map) {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (!isFrameGraphics(child as SuikaGraphics) || !selectedChildSet.has(child as SuikaFrame)) {
        continue;
      }

      const leftSortIndex = children[i - 1]?.getSortIndex() ?? null;
      const rightSortIndex = child?.getSortIndex() ?? null;
...
const flatFrame = (
  ...
  for (let i = 0; i < children.length; i++) {
    const child = children[i];
    const worldTf = child?.getWorldTransform() ?? { x: 0, y: 0, scaleX: 1, scaleY: 1, rotate: 0, skewX: 0, skewY: 0 };

    transaction.recordOld(child?.attrs.id ?? '', {
      parentIndex: cloneDeep(child?.attrs.parentIndex ?? { guid: '', position: '' }),
      transform: cloneDeep(child?.attrs.transform ?? [1, 0, 0, 1, 0, 0]),
    });
    ...
    child?.setWorldTransform(worldTf as unknown as IMatrixArr);
