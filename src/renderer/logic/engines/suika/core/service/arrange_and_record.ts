import { cloneDeep, swap } from '../../common';
...
const forward = (
  ...
  for (const [parent, movedChildSet] of map) {
    ...
    for (let i = index; i >= 0; i--) {
      ...
      if (movedChildSet.has(child as SuikaGraphics)) {
        const newLeft = children[i + 1]?.getSortIndex() ?? null;
  ...
  for (const [parent, movedChildSet] of map) {
    ...
    for (let i = index; i <= children.length; i++) {
      ...
      if (movedChildSet.has(child as SuikaGraphics)) {
        ...
        const newRight = children[i - 1]?.getSortIndex() ?? null;
        const newIndex = generateKeyBetween(newLeft, newRight);
        swap(children, i, i - 1);
...
const getFirstUnmovedGraphics = (
  ...
  for (; index < graphicsArr.length; index++) {
    if (!movedGraphSet.has(graphicsArr[index] as SuikaGraphics)) {
      break;
    }
    count++;
  }
