...
const isAlreadyAligned = (bboxes: IBox[], type: AlignType): boolean => {
  ...
  const referenceValue = alignmentFunction(rects[0] as IRect);
  ...
  switch (type) {
    case AlignType.Left: {
      for (let i = 0; i < graphicsArr.length; i++) {
        ...
        const dx = mixedBBox.minX - (bboxes[i]?.minX ?? 0);
        if (dx !== 0) {
          updateGraphicsPosition(graphics as SuikaGraphics, worldTfs[i] as IMatrixArr, dx, 0);
        }
      }
      break;
    }
    case AlignType.HCenter: {
      ...
      for (let i = 0; i < graphicsArr.length; i++) {
        const graphics = graphicsArr[i];
        const dx = centerX - ((bboxes[i]?.minX ?? 0) / 2 + (bboxes[i]?.maxX ?? 0) / 2);
        if (dx !== 0) {
          updateGraphicsPosition(graphics as SuikaGraphics, worldTfs[i] as IMatrixArr, dx, 0);
        }
      }
    ...
    case AlignType.Right: {
      for (let i = 0; i < graphicsArr.length; i++) {
        const graphics = graphicsArr[i];
        const dx = mixedBBox.maxX - (bboxes[i]?.maxX ?? 0);
        if (dx !== 0) {
          updateGraphicsPosition(graphics as SuikaGraphics, worldTfs[i] as IMatrixArr, dx, 0);
        }
      }
    ...
    case AlignType.Top: {
      for (let i = 0; i < graphicsArr.length; i++) {
        const graphics = graphicsArr[i];
        const dy = mixedBBox.minY - (bboxes[i]?.minY ?? 0);
        if (dy !== 0) {
          updateGraphicsPosition(graphics as SuikaGraphics, worldTfs[i] as IMatrixArr, 0, dy);
        }
      }
      break;
    }
    case AlignType.VCenter: {
      ...
      for (let i = 0; i < graphicsArr.length; i++) {
        const graphics = graphicsArr[i];
        const dy = centerY - ((bboxes[i]?.minY ?? 0) / 2 + (bboxes[i]?.maxY ?? 0) / 2);
        if (dy !== 0) {
          updateGraphicsPosition(graphics as SuikaGraphics, worldTfs[i] as IMatrixArr, 0, dy);
        }
      }
    ...
    case AlignType.Bottom: {
      for (let i = 0; i < graphicsArr.length; i++) {
        const graphics = graphicsArr[i];
        const dy = mixedBBox.maxY - (bboxes[i]?.maxY ?? 0);
        if (dy !== 0) {
          updateGraphicsPosition(graphics as SuikaGraphics, worldTfs[i] as IMatrixArr, 0, dy);
        }
      }
