...
export const simplePath = (segs: ISegment[], tol: number) => {
  ...
  for (let i = 0, len = curves.length; i <= len; i++) {
    ...
    const outPt = curve
      ? {
          x: (curve[1] as any)[0] - point.x!,
          y: (curve[1] as any)[1] - point.y!,
        }
      : { x: 0, y: 0 };
    const inPt = prevCurve
      ? {
          x: (prevCurve[2] as any)[0] - point.x!,
          y: (prevCurve[2] as any)[1] - point.y!,
        }
      : { x: 0, y: 0 };
    newSegs.push({
...
export const insertPathSeg = (
  ...
  if (!leftSeg || !rightSeg) return pathItem;
...
export const deletePathSegAndHeal = (
  ...
  if (!midSeg || !leftSeg || !rightSeg) return pathItem;
