...
export class RadixUIPerformanceMonitor {
  ...
  private recordRender(componentName: string, renderTime: number): void {
    ...
    // 记录调试信息
    console.debug(
      `[radix-performance] ${componentName}渲染完成`,
      {
  ...
  private createAlert(alert: PerformanceAlert): void {
    ...
    // 记录警报
    console.warn(
      `[radix-performance-alert] ${alert.message}`,
      {
        type: alert.type,
  ...
  private attemptAutoFix(alert: PerformanceAlert): void {
    switch (alert.type) {
      case 'infinite_loop':
        console.error(
          `[radix-auto-fix] 尝试修复${alert.componentName}的无限循环问题`,
          { alert },
          'RadixUIPerformanceMonitor'
        );
        ...
        // 触发状态重置（简化处理）
        console.warn('检测到无限循环，建议检查组件状态更新逻辑');
        break;
  ...
  public setMonitoringEnabled(enabled: boolean): void {
    ...
    console.info(
      `[radix-monitoring] Radix UI性能监控${enabled ? '启用' : '禁用'}`,
      { enabled },
      'RadixUIPerformanceMonitor'
    );
  }
