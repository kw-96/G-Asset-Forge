...
export const useRadixUIPerformance = (
  ...
  const startRenderMeasurement = useCallback(() => {
    ...
    if (debugMode) {
      console.debug(
        `[radix-performance-hook] 开始测量${componentName}渲染性能`,
  ...
  const endRenderMeasurement = useCallback(() => {
    ...
    if (timeSinceLastRender < 16 && renderCountRef.current > alertThreshold) {
      console.warn(
        `[radix-performance-hook] ${componentName}渲染频率异常`,
        {
          renderCount: renderCountRef.current,
          timeSinceLastRender,
          alertThreshold,
    ...
    if (debugMode) {
      console.debug(
        `[radix-performance-hook] 完成测量${componentName}渲染性能`,
        { 
          renderCount: renderCountRef.current,
          timeSinceLastRender,
        },
  ...
  const resetMetrics = useCallback(() => {
    ...
    if (debugMode) {
      console.info(
        `[radix-performance-hook] 重置${componentName}性能指标`,
        {},
        'useRadixUIPerformance'
      );
    }
...
export const useRadixUIRenderCount = (componentName: string, enabled: boolean = true) => {
  ...
  useEffect(() => {
    ...
    if (now - lastLogTimeRef.current > 5000) {
      console.info(
        `[radix-render-count] ${componentName}渲染统计`,
        {
          renderCount: renderCountRef.current,
...
export const useRadixUIAnomalyDetection = (
  ...
  useEffect(() => {
    ...
    if (renderTimesRef.current.length > maxRenderCount && !anomalyDetectedRef.current) {
      ...
      console.error(
        `[radix-anomaly-detection] ${componentName}检测到异常渲染模式`,
        {
