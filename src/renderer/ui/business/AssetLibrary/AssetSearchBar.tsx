...
import { 
  type AssetSearchSuggestion,
  type AssetSearchOptions
} from '../../../logic/managers/assets/AssetSearchEngine';

interface IAssetSearchBarProps {
  onSearch: (options: Partial<AssetSearchOptions>) => void;
  onSuggestionSelect?: (suggestion: AssetSearchSuggestion) => void;
  getSuggestions?: (query: string) => Promise<AssetSearchSuggestion[]>;
  placeholder?: string;
  className?: string;
...
export const AssetSearchBar: React.FC<IAssetSearchBarProps> = ({
  ...
  const [suggestions, setSuggestions] = useState<AssetSearchSuggestion[]>([]);
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    if (!showSuggestions || suggestions.length === 0) {
      if (e.key === 'Enter') {
        onSearch({ searchQuery: query.trim() });
        setShowSuggestions(false);
      }
      return;
    }
  ...
  // 处理建议点击
  const handleSuggestionClick = useCallback((suggestion: AssetSearchSuggestion) => {
    setQuery(suggestion.text);
    setShowSuggestions(false);
    setSelectedSuggestionIndex(-1);
    
    onSearch({ searchQuery: suggestion.text });
  ...
  // 获取建议类型图标
  const getSuggestionIcon = (type: AssetSearchSuggestion['type']): string => {
    switch (type) {
      case 'keyword': return '🔍';
      case 'tag': return '🏷️';
  ...
  // 获取建议类型名称
  const getSuggestionTypeName = (type: AssetSearchSuggestion['type']): string => {
    switch (type) {
