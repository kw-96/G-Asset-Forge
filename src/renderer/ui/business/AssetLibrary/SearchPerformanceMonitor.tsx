...
import { type SearchAnalytics } from '../../../logic/managers/assets/AssetSearchEngine';
...
export const SearchPerformanceMonitor: React.FC<ISearchPerformanceMonitorProps> = ({
  ...
  const performanceLevel = getPerformanceLevel(analytics.searchTimes.get('averageTime') || 0);
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ fontSize: '12px', color: '#666' }}>
            {analytics.searchTimes.size} 次搜索
          </span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '12px' }}>
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontWeight: 'bold', color: performanceLevel.color }}>
              {formatTime(analytics.searchTimes.get('averageTime') || 0)}
            </div>
          </div>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontWeight: 'bold' }}>
              {Math.round(analytics.searchResults.get('averageResultCount') || 0)}
            </div>
          </div>
          
          <div style={{ textAlign: 'center' }}>
            <div style={{ color: '#666', marginBottom: '2px' }}>慢查询</div>
            <div style={{ fontWeight: 'bold', color: Array.isArray(analytics.searchTimes.get('slowQueries')) && (analytics.searchTimes.get('slowQueries') as unknown as any[])?.length > 0 ? '#dc3545' : '#28a745' }}>
              {(analytics.searchTimes.get('slowQueries') as unknown as any[])?.length || 0}
            </div>
          </div>
        </div>
      </div>
      ...
      {isExpanded && (
        <div style={{ borderTop: '1px solid #e0e0e0' }}>
          ...
          <div style={{ padding: '16px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '16px' }}>
            ...
            {/* 慢查询 */}
            {Array.isArray(analytics.searchTimes.get('slowQueries')) && (analytics.searchTimes.get('slowQueries') as unknown as any[])?.length > 0 && (
              <div>
                <h5 style={{ margin: '0 0 8px 0', fontSize: '13px', fontWeight: 'bold', color: '#dc3545' }}>
                  慢查询 (&gt;100毫秒)
                </h5>
                <div style={{ fontSize: '11px' }}>
                  {(analytics.searchTimes.get('slowQueries') as unknown as any[])?.map((slowQuery: any, index: number) => (
                    <div key={index} style={{
                      display: 'flex',
                      justifyContent: 'space-between',
                      padding: '4px 0',
          ...
          {/* 性能建议 */}
          {typeof analytics.searchTimes.get('averageTime') === 'number' && 
            (analytics.searchTimes.get('averageTime') as number) > 100 && (
            <div style={{
              margin: '16px',
              padding: '12px',
