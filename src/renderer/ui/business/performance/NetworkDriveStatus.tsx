...
interface NetworkDriveStatusProps {
  className?: string;
  manager: any; // 临时使用 any 类型，因为 NetworkDriveManager 的实际类型定义不明确
}
...
export const NetworkDriveStatus: React.FC<NetworkDriveStatusProps> = ({
  ...
  useEffect(() => {
    const updateDriveStatuses = async () => {
      try {
        // 检查 manager 是否有所需的方法
        if (!manager || typeof manager.getAllDrives !== 'function') {
          console.warn('NetworkDriveManager 实例无效或缺少 getAllDrives 方法');
          return;
        }
        ...
        for (const drive of drives) {
          try {
            // 检查 testDriveConnection 方法是否存在
            if (typeof manager.testDriveConnection !== 'function') {
              console.warn('NetworkDriveManager 缺少 testDriveConnection 方法');
              continue;
            }
    ...
    <div className={`bg-white border border-gray-200 rounded-lg shadow-sm ${className}`}>
      ...
      >
        <div className="flex items-center space-x-2">
          <div className={`w-2 h-2 rounded-full ${overallAvailability >= 99 ? 'bg-green-500' :
            ...
            <div className="space-y-1">
              {driveStatuses.map((drive) => (
                <div key={drive.id} className="flex items-center justify-between text-xs">
                  <div className="flex items-center space-x-2">
                    <span className={`w-1 h-1 rounded-full ${drive.isAvailable ? 'bg-green-500' : 'bg-red-500'
                      }`}></span>
