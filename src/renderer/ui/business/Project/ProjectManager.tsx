...
interface IProjectManagerProps {
  onProjectOpen?: (project: ProjectData) => void;
  onProjectCreate?: (project: ProjectData) => void;
  onProjectClose?: () => void;
  className?: string;
...
export const ProjectManager: React.FC<IProjectManagerProps> = ({
  ...
  useEffect(() => {
    ...
    projectManagerRef.current.initialize().catch(error => {
      console.error('初始化 ProjectManager 失败:', error);
      setError(`初始化失败: ${error.message}`);
    });

    // 绑定事件
    projectManagerRef.current.on('project-created', (_event, data) => {
      setCurrentProject(data.project);
      onProjectCreate?.(data.project);
    });

    projectManagerRef.current.on('project-opened', (_event, data) => {
      setCurrentProject(data.project);
      onProjectOpen?.(data.project);
    });

    projectManagerRef.current.on('project-closed', (_event, _data) => {
      setCurrentProject(null);
      onProjectClose?.();
    });

    // 注意：recentProjectsUpdated 事件可能不存在，需要手动获取
    // projectManagerRef.current.on('recentProjectsUpdated', (event, data) => {
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
            <div>
              ...
              <div style={{ fontSize: '11px', color: '#999' }}>
                创建于: {formatDate(new Date(currentProject.metadata.createdAt))} • 
                更新于: {formatDate(new Date(currentProject.metadata.updatedAt))}
            ...
            {currentProject.metadata.thumbnailUrl && (
              <img
                src={currentProject.metadata.thumbnailUrl}
                alt="项目缩略图"
                style={{
                  width: '60px',
                  height: '60px',
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '10px', color: '#999' }}>
                    {project.lastOpenedAt ? 
                      `最后打开: ${formatDate(new Date(project.lastOpenedAt))}` : 
                      `创建于: ${formatDate(new Date(project.createdAt))}`
                    }
                    {project.filePath && (
                      <> • {project.filePath}</>
                    )}
