...
import { StableDropdown, type StableDropdownItemProps } from '../../molecules/Dropdown/Dropdown';
import { canvasEvents } from '../../../../logic/utils/events/canvasEvents';
// import { Badge } from '../../Badge/Badge.tsx';
import { SettingsModal } from '../../../business/Settings/SettingsModal';
// import { EnhancedIconButton } from '../Enhanced/EnhancedIconButton';
import { WindowControls } from '../../../business/Layout/WindowControls';
...
export const TopToolbar: React.FC<TopToolbarProps> = ({
  ...
  useEffect(() => {
    try {
      ...
      // 初始化项目管理器
      projectManagerRef.current.initialize().catch(error => {
        console.warn('初始化 ProjectManager 失败:', error);
      });
    } catch (e) {
  ...
  const handleNewTab = useCallback(async () => {
    try {
      ...
      if (projectManagerRef.current) {
        const result = await projectManagerRef.current.createProject({ name: '无标题' });
        if (result.success && result.data?.project) {
          project = result.data.project as ProjectData;
        } else {
          console.error('创建项目失败:', result.message);
          project = createEmptyProject('无标题') as unknown as ProjectData;
        }
      } else {
        project = createEmptyProject('无标题') as unknown as ProjectData;
      }
      const id = `tab-${Date.now()}`;
      setTabs(prev => [...prev, { id, title: project?.metadata.name || '无标题', project }]);
      setActiveTabId(id);
      if (project) {
  ...
  const handleOpenProjectToTab = useCallback(async () => {
    try {
      ...
      const result = await projectManagerRef.current.loadProject(res.data!.filePaths[0]!);
      if (result.success && result.data?.project) {
        const loaded = result.data.project as ProjectData;
        const id = `tab-${Date.now()}`;
   ...
   * 获取当前激活标签的项目
   */
  const getActiveProject = useCallback((): ProjectData | null => {
    const active = tabs.find(t => t.id === activeTabId);
    return (active?.project as ProjectData) || null;
  }, [tabs, activeTabId]);
  ...
  const handleCloseTab = useCallback((id: string) => {
    setTabs(prev => {
      ...
      const fallback = createEmptyProject('无标题') as unknown as ProjectData;
  ...
  const unifiedMenuItems: StableDropdownItemProps[] = [
    // 文件
    { id: 'file__new', label: '新建项目', group: '文件', shortcut: 'Ctrl+N', children: <></>, onSelect: () => { void handleNewTab(); } },
    { id: 'file__open', label: '打开项目', group: '文件', shortcut: 'Ctrl+O', children: <></>, onSelect: () => { void handleOpenProjectToTab(); } },
    { id: 'file__save', label: '保存项目', group: '文件', shortcut: 'Ctrl+S', children: <></>, onSelect: () => { void handleSaveProjectDefault(); } },
    { id: 'file__export', label: '另存为(.gaf)', group: '文件', shortcut: 'Ctrl+E', children: <></>, onSelect: () => { void handleExportProjectAs(); } },

    // 编辑
    { id: 'edit__undo', label: '撤销', group: '编辑', shortcut: 'Ctrl+Z', children: <></>, onSelect: () => handleEditAction('undo') },
    { id: 'edit__redo', label: '重做', group: '编辑', shortcut: 'Ctrl+Shift+Z', children: <></>, onSelect: () => handleEditAction('redo') },
    { id: 'edit__copy', label: '复制', group: '编辑', shortcut: 'Ctrl+C', children: <></>, onSelect: () => handleEditAction('copy') },
    { id: 'edit__paste', label: '粘贴', group: '编辑', shortcut: 'Ctrl+V', children: <></>, onSelect: () => handleEditAction('paste') },

    // 画布
    { id: 'canvas__fit', label: '适应内容', group: '画布', shortcut: 'Ctrl+1', children: <></>, onSelect: () => canvasEvents.emit('fitToContent', () => {}) },
    { id: 'canvas__reset', label: '重置视图', group: '画布', shortcut: 'Ctrl+0', children: <></>, onSelect: () => canvasEvents.emit('resetView', () => {}) },
    { id: 'canvas__zoom_in', label: '放大', group: '画布', shortcut: 'Ctrl++', children: <></>, onSelect: () => canvasEvents.emit('zoomIn', () => {}) },
    { id: 'canvas__zoom_out', label: '缩小', group: '画布', shortcut: 'Ctrl+-', children: <></>, onSelect: () => canvasEvents.emit('zoomOut', () => {}) },
    { id: 'canvas__toggle_grid', label: '显示/隐藏网格', group: '画布', shortcut: 'G', children: <></>, onSelect: () => canvasEvents.emit('toggleGrid', () => {}) },
    { id: 'canvas__toggle_ruler', label: '显示/隐藏标尺', group: '画布', shortcut: 'Shift+R', children: <></>, onSelect: () => canvasEvents.emit('toggleRuler', () => {}) },
    { id: 'canvas__toggle_guides', label: '显示/隐藏参考线', group: '画布', shortcut: 'R', children: <></>, onSelect: () => canvasEvents.emit('toggleGuides', () => {}) },
      ...
      <ToolbarSection $leftDivider $noGap>
        ...
        <NoDrag>
          <StableDropdown
            ...
            children={unifiedMenuItems as any}
            side="bottom"
            align="end"
          />
        </NoDrag>
