...
import { ErrorAnalysisResult, ErrorPattern, ErrorCategory } from './ErrorAnalyzer';
...
export class ErrorRecoveryManager {
  ...
  public async executeRecovery(
    ...
    try {
      ...
      reactLoopFixToolkit.debugLogger.info(
        'recovery-manager',
        `恢复策略执行成功: ${strategy.action}`,
        { errorId, duration, action: strategy.action },
        'ErrorRecoveryManager'
      );
      ...
      reactLoopFixToolkit.debugLogger.error(
        'recovery-manager',
        `恢复策略执行失败: ${strategy.action}`,
        { errorId, duration, error: recoveryResult.error?.message },
        'ErrorRecoveryManager'
      );
   ...
   * 生成恢复策略
   */
  private generateRecoveryStrategies(errorAnalysis: ErrorAnalysisResult): RecoveryStrategy[] {
   ...
   * 获取模式特定的策略
   */
  private getPatternSpecificStrategies(pattern: ErrorPattern): RecoveryStrategy[] {
    switch (pattern) {
   ...
   * 获取类别特定的策略
   */
  private getCategorySpecificStrategies(category: ErrorCategory): RecoveryStrategy[] {
    switch (category) {
   ...
   * 获取通用策略
   */
  private getGenericStrategies(): RecoveryStrategy[] {
    return [
      {
  ...
  private selectRecommendedStrategy(
    strategies: RecoveryStrategy[],
    _errorAnalysis: ErrorAnalysisResult
  ): RecoveryStrategy {
    // 优先选择高优先级、低风险的策略
   ...
   * 选择备用策略
   */
  private selectFallbackStrategy(strategies: RecoveryStrategy[]): RecoveryStrategy {
    // 备用策略通常是重新加载页面
   ...
   * 判断是否应该启用自动恢复
   */
  private shouldEnableAutoRecovery(errorAnalysis: ErrorAnalysisResult): boolean {
    // 高置信度且可恢复的错误启用自动恢复
   ...
   * 获取默认最大重试次数
   */
  private getDefaultMaxRetries(errorAnalysis: ErrorAnalysisResult): number {
   ...
   * 获取恢复历史
   */
  public getRecoveryHistory(errorId?: string): Map<string, RecoveryResult[]> | RecoveryResult[] {
    if (errorId) {
   ...
   * 清除恢复历史
   */
  public clearRecoveryHistory(errorId?: string): void {
    if (errorId) {
  ...
  public getRecoveryStats(): {
    ...
    let totalDuration = 0;
    const actionCounts = new Map<RecoveryAction, number>();
