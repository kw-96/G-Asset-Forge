...
import { type IImageExportOptions } from './ImageExportEngine';
import { type IH5Page } from '../types';

export interface IExportOptimizationSuggestion {
  type: 'warning' | 'info' | 'error';
  category: 'fileSize' | 'performance' | 'quality' | 'compatibility';
  message: string;
  suggestion: string;
  impact: 'low' | 'medium' | 'high';
  autoFix?: () => Partial<IImageExportOptions>;
}

export interface IExportPreviewOptions {
...
export interface IExportOptimizerEvents extends Record<string, (...args: any[]) => void> {
  previewReady(previewUrl: string, metrics: IExportPerformanceMetrics): void;
  optimizationSuggestions(suggestions: IExportOptimizationSuggestion[]): void;
...
export class ExportOptimizer {
  private emitter = new EventEmitter<IExportOptimizerEvents>();
  private previewOptions: IExportPreviewOptions;
  ...
  constructor(previewOptions?: Partial<IExportPreviewOptions>) {
    this.previewOptions = {
  ...
  analyzeExportOptions(
    ...
    page?: IH5Page
  ): IExportOptimizationSuggestion[] {
    const suggestions: IExportOptimizationSuggestion[] = [];
  ...
  async generatePreview(
    ...
    this.previewDebounceTimer = setTimeout(async () => {
      try {
        ...
        // 计算性能指标
        const metrics: IExportPerformanceMetrics = {
   ...
   * 检查性能警告
   */
  private checkPerformanceWarnings(metrics: IExportPerformanceMetrics): void {
  ...
  async benchmarkExportPerformance(
    canvas: HTMLCanvasElement,
    options: IImageExportOptions
  ): Promise<IExportPerformanceMetrics> {
    const startTime = performance.now();
   ...
   * 更新预览选项
   */
  updatePreviewOptions(options: Partial<IExportPreviewOptions>): void {
   ...
   * 事件管理
   */
  on<T extends keyof IExportOptimizerEvents>(eventName: T, listener: IExportOptimizerEvents[T]): void {
    this.emitter.on(eventName, listener);
  }

  off<T extends keyof IExportOptimizerEvents>(eventName: T, listener: IExportOptimizerEvents[T]): void {
