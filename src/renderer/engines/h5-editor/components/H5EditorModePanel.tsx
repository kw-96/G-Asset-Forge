...
interface IH5EditorModePanelProps {
  ...
  onProjectChange?: (project: IH5Project | null) => void;
  className?: string;
...
export const H5EditorModePanel: React.FC<IH5EditorModePanelProps> = ({
  ...
  const [currentProject, setCurrentProject] = useState<IH5Project | null>(null);
  const [pages, setPagesInfo] = useState<Array<any>>([]);
  ...
  // 项目变更回调
  const handleProjectChange = useCallback((project: IH5Project) => {
  ...
  // 页面变更回调
  const handlePageChange = useCallback((page: IH5Page) => {
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              {pages.map(page => (
                <div 
                  ...
                  style={{
                    padding: '8px',
                    border: page.isCurrent ? '2px solid #007bff' : '1px solid #ddd',
                    borderRadius: '4px',
                    backgroundColor: page.isCurrent ? '#e3f2fd' : 'white',
                    fontSize: '12px',
                    minWidth: '120px'
                  }}
                >
                  ...
                  <div style={{ color: '#666' }}>
                    {page.componentCount} 个组件
                  </div>
                  {!page.isCurrent && pages.length > 1 && (
                    <button 
