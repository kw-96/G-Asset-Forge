...
export interface IH5ComponentLibraryItem {
  id: string;
  name: string;
  type: IH5Component['type'];
  category: string;
  thumbnail: string;
  defaultProps: Record<string, any>;
  previewComponent: IH5Component;
}
...
export interface IH5EditorEvents extends Record<string, (...args: any[]) => void> {
  ...
  backgroundChange(background: IH5Page['background']): void;
  canvasSizeChange(size: { width: number; height: number }): void;
  templateLoad(template: IH5Template): void;
  exportStart(): void;
...
export class H5Editor {
  ...
  private templates: Map<string, IH5Template> = new Map();
  private componentLibrary: Map<string, IH5ComponentLibraryItem> = new Map();
  ...
  private initializeComponentLibrary(): void {
    ...
    // 初始化基础组件库
    const basicComponents: IH5ComponentLibraryItem[] = [
      {
        id: 'text-basic',
        name: '基础文本',
        type: 'text',
  ...
  private initializeTemplates(): void {
    ...
    // 初始化基础模板
    const basicTemplates: IH5Template[] = [
      {
        id: 'mobile-basic',
        name: '移动端基础模板',
        category: '移动端',
  ...
  // 页面管理
  createPage(name: string, options?: Partial<IH5Page>): IH5Page {
    const page: IH5Page = {
  ...
  duplicatePage(pageId: string, newName?: string): IH5Page | null {
    const sourcePage = this.pages.get(pageId);
    if (!sourcePage) return null;

    const duplicatedPage: IH5Page = {
      ...sourcePage,
      id: 'page_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9),
  ...
  getComponent(componentId: string): IH5Component | null {
    if (!this.currentPage) return null;
    return this.currentPage.components.find(c => c.id === componentId) || null;
  }

  getComponentsByType(type: IH5Component['type']): IH5Component[] {
    if (!this.currentPage) return [];
  ...
  // 背景设置
  setPageBackground(background: IH5Page['background'] | any): void {
    if (!this.currentPage) return;
  ...
  saveAsTemplate(name: string, category: string = '自定义'): IH5Template {
    const template: IH5Template = {
  ...
  getTemplatesByCategory(category: string): IH5Template[] {
    return Array.from(this.templates.values()).filter(t => t.category === category);
  }

  // 组件库管理
  getComponentLibrary(): IH5ComponentLibraryItem[] {
    return Array.from(this.componentLibrary.values());
  }

  getComponentLibraryByCategory(category: string): IH5ComponentLibraryItem[] {
    return Array.from(this.componentLibrary.values()).filter(item => item.category === category);
  }

  addToComponentLibrary(item: IH5ComponentLibraryItem): void {
  ...
  private renderImagePlaceholder(component: IH5Component, text: string): void {
    // 绘制占位符背景
    this.ctx.fillStyle = '#f0f0f0';
  ...
  private renderShapeComponent(component: IH5Component): void {
    const { 
      shapeType, 
      fillColor, 
      strokeColor, 
  ...
  private renderContainerComponent(component: IH5Component): void {
  ...
  private renderDefaultComponent(component: IH5Component): void {
    // 默认渲染为带边框的矩形
    this.ctx.fillStyle = '#f0f0f0';
    this.ctx.fillRect(0, 0, component.width, component.height);
    this.ctx.strokeStyle = '#ccc';
  ...
  private renderLockIndicator(component: IH5Component): void {
    const size = 16;
  ...
  exportAllPagesData(): IH5Page[] {
    return Array.from(this.pages.values()).map(page => 
      JSON.parse(JSON.stringify(page))
    );
  }

  exportProjectData(): { pages: IH5Page[]; metadata: any } {
    return {
