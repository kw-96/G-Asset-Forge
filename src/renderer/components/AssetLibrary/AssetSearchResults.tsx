...
import { type IAssetMetadata, type IAssetCategoryInfo } from '../../managers/assets/AssetLibraryManager';

interface IAssetSearchResultsProps {
  assets: IAssetMetadata[];
  categories: IAssetCategoryInfo[];
  viewMode: 'grid' | 'list';
  selectedAsset?: IAssetMetadata | null;
  isLoading?: boolean;
  onAssetClick?: (asset: IAssetMetadata) => void;
...
export const AssetSearchResults: React.FC<IAssetSearchResultsProps> = ({
  ...
  // 处理收藏切换
  const handleToggleFavorite = useCallback((asset: IAssetMetadata, e: React.MouseEvent) => {
    e.stopPropagation();
      ...
      {assets.map(asset => (
        <div
          ...
          style={{
            ...
            border: selectedAsset?.id === asset.id ? '2px solid #007bff' : '1px solid #e0e0e0',
            borderRadius: '8px',
        ...
        >
          ...
          <div style={{ 
            ...
            <div>
              ...
              {asset.description && viewMode === 'list' && (
                <div style={{ 
                  fontSize: '12px', 
                  color: '#666', 
                  marginBottom: '4px',
            ...
            <div style={{ fontSize: '10px', color: '#666' }}>
              <div style={{ marginBottom: '2px' }}>
                ...
                {viewMode === 'list' && ` • ${formatFileSize(asset.fileSize)}`}
              </div>
              
              {viewMode === 'list' && (
                <div style={{ marginBottom: '2px' }}>
                  评分: {'⭐'.repeat(Math.floor(asset.rating))} {asset.rating.toFixed(1)} • 
            ...
            {asset.tags.length > 0 && (
              <div style={{ 
                ...
                {asset.tags.slice(0, viewMode === 'grid' ? 3 : 5).map(tag => (
                  <span
                    key={tag}
                    style={{
                      padding: '1px 4px',
                ...
                {asset.tags.length > (viewMode === 'grid' ? 3 : 5) && (
                  <span style={{
                    padding: '1px 4px',
