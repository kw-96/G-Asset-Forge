...
interface IAdvancedSearchPanelProps {
  ...
  currentFilter: IAdvancedFilter;
  onFilterChange: (filter: IAdvancedFilter) => void;
  onReset: () => void;
  onClose?: () => void;
  className?: string;
...
export const AdvancedSearchPanel: React.FC<IAdvancedSearchPanelProps> = ({
  ...
  const getActiveFilterCount = (): number => {
    ...
    if (currentFilter.author) count++;
    if (currentFilter.isFavorite !== undefined) count++;
    if (currentFilter.isCustom !== undefined) count++;
    if (currentFilter.hasPreview !== undefined) count++;
    if (currentFilter.hasThumbnail !== undefined) count++;
    ...
    if (currentFilter.dimensionRatio?.min || currentFilter.dimensionRatio?.max) count++;
    if (currentFilter.ratingRange?.min || currentFilter.ratingRange?.max) count++;
    if (currentFilter.downloadCountRange?.min || currentFilter.downloadCountRange?.max) count++;
            ...
            {currentFilter.category && (
              <div>
                ...
                >
                  ...
                  {(categories.find(c => c.id === currentFilter.category)?.subcategories ?? []).map(sub => (
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {['free', 'premium', 'custom'].map(license => (
                  <button
                    ...
                    style={{
                      padding: '6px 12px',
                      border: `1px solid ${(currentFilter.license || []).includes(license as any) ? '#007bff' : '#ddd'}`,
                      backgroundColor: (currentFilter.license || []).includes(license as any) ? '#007bff' : 'white',
                      color: (currentFilter.license || []).includes(license as any) ? 'white' : '#666',
                      borderRadius: '16px',
                      cursor: 'pointer',
                      fontSize: '12px'
                    }}
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {['image/png', 'image/jpeg', 'image/gif', 'image/svg+xml', 'image/webp'].map(fileType => (
                  <button
                    ...
                    style={{
                      padding: '6px 12px',
                      border: `1px solid ${(currentFilter.fileType || []).includes(fileType) ? '#28a745' : '#ddd'}`,
                      backgroundColor: (currentFilter.fileType || []).includes(fileType) ? '#28a745' : 'white',
                      color: (currentFilter.fileType || []).includes(fileType) ? 'white' : '#666',
                      borderRadius: '16px',
                      cursor: 'pointer',
                      fontSize: '12px'
                    }}
