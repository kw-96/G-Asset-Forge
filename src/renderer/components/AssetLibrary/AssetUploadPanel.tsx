...
interface IAssetUploadPanelProps {
  categories: IAssetCategoryInfo[];
  onUpload: (assets: IUploadAssetData[]) => Promise<void>;
  onClose?: () => void;
...
export const AssetUploadPanel: React.FC<IAssetUploadPanelProps> = ({
  categories,
  onUpload,
  onClose,
  ...
  const [uploadData, setUploadData] = useState<Map<string, Partial<IUploadAssetData>>>(new Map());
  ...
  // 自动识别分类和标签
  const autoDetectMetadata = useCallback((file: File): { category: AssetCategory; tags: string[] } => {
  ...
  // 更新上传数据
  const updateUploadData = useCallback((fileName: string, updates: Partial<IUploadAssetData>) => {
    setUploadData(prev => {
  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      const assetsToUpload: IUploadAssetData[] = [];
      ...
      for (const file of selectedFiles) {
        ...
        if (data && data.name && data.category) {
          assetsToUpload.push({
            file,
            name: data.name,
            // exactOptionalPropertyTypes: 仅在存在时提供可选字段
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => updateUploadData(file.name, { category: e.target.value as AssetCategory })}
                        disabled={isUploading}
                        style={{
                          width: '100%',
                          padding: '4px 6px',
