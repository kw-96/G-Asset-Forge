...
export const RulerGuides: React.FC<RulerGuidesProps> = ({ mode = 'design' }) => {
  ...
  React.useEffect(() => {
    ...
    const offAdd = canvasEvents.on('addGuide', (type, pos) => setGuides(prev => [...prev, { id: `${Date.now()}`, type, position: pos }]));
    const offMove = canvasEvents.on('moveGuide', (id, pos) => setGuides(prev => prev.map(g => g.id === id ? { ...g, position: pos } : g)));
    const offRemove = canvasEvents.on('removeGuide', (id) => setGuides(prev => prev.filter(g => g.id !== id)));
    return () => {
