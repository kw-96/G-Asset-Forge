...
import { type GAssetForgeDocument } from '../document';
...
export class GAssetForgeGraphics<ATTRS extends GraphicsAttrs = GraphicsAttrs> {
  type = GraphicsType.Graph;
  attrs: ATTRS;
  protected doc: GAssetForgeDocument;
  ...
  static dMove(graphicsArr: GAssetForgeGraphics[], dx: number, dy: number) {
  ...
  protected children: GAssetForgeGraphics[] = [];
  ...
  setChildren(graphs: GAssetForgeGraphics[]) {
    if (!this.isContainer) {
      return;
    }
  ...
  insertChild(graphics: GAssetForgeGraphics, sortIdx?: string) {
    if (!this.isContainer) {
  ...
  removeChild(graphics: GAssetForgeGraphics) {
  ...
  sortChildren() {
    GAssetForgeGraphics.sortGraphicsArray(this.children);
  }

  static sortGraphicsArray(graphicsArr: GAssetForgeGraphics[]) {
  ...
  getSortIndexPath() {
    ...
    let node: GAssetForgeGraphics | undefined = this;
    while (node) {
  ...
  static sortGraphics(graphics: GAssetForgeGraphics[]) {
  ...
  forEachVisibleChildNode(callback: (graphics: GAssetForgeGraphics) => void) {
    if (!this.isVisible()) {
      return;
    }
    for (const child of this.children) {
