import { cloneDeep, parseHexToRGBA, parseRGBAStr } from '@g-asset-forge/common';
import {
  commandsToStr,
...
export class GAssetForgePath extends GAssetForgeGraphics<PathAttrs> {
  ...
  static recomputeAttrs(pathData: IPathItem[], transform: IMatrixArr) {
    const rect = GAssetForgePath.computeRect(pathData);
  ...
  override updateAttrs(
    ...
    if (partialAttrs.pathData) {
      ...
      partialAttrs = {
        ...partialAttrs,
        ...GAssetForgePath.recomputeAttrs(partialAttrs.pathData!, transform),
      };
    }
    ...
    const draw = (layerCtx: CanvasRenderingContext2D) => {
      ...
      for (const pathItem of pathData) {
        ...
        for (let i = 1; i <= segs.length; i++) {
          ...
          const handle1 = GAssetForgePath.getHandleOut(prevSeg);
          const handle2 = GAssetForgePath.getHandleIn(currSeg);
          if (!handle1 && !handle2) {
  ...
  getSeg(
    ...
    let seg = GAssetForgePath.getSeg(this.attrs.pathData, pathIdx, segIdx);
    seg = cloneDeep(seg);
