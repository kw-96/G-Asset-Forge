import { cloneDeep, swap } from '@g-asset-forge/common';
...
export const arrangeAndRecord = (editor: GAssetForgeEditor, type: ArrangeType) => {
  ...
  // parent -> children set
  const map = new Map<GAssetForgeGraphics, Set<GAssetForgeGraphics>>();
...
const front = (
  ...
  map: Map<GAssetForgeGraphics, Set<GAssetForgeGraphics>>,
) => {
  ...
  for (const [parent, movedChildSet] of map) {
    ...
    const sortedMovedChildren = GAssetForgeGraphics.sortGraphicsArray(
...
const getFirstUnmovedGraphics = (
  ...
  movedGraphSet: Set<GAssetForgeGraphics>,
