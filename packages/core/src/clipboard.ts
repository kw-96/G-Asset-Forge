...
export class ClipboardManager {
  ...
  private async createGraphicsWithImg(imgUrl: string) {
    ...
    if (img) {
      const rectGraphics = new GAssetForgeRect(
        {
          objectName: '',
          width: img.width,
          height: img.height,
  ...
  private getSelectedItemsSnapshot() {
    const selectedItems = GAssetForgeGraphics.sortGraphics(
  ...
  private addGraphsFromClipboard(dataStr: string, point?: IPoint) {
    ...
    if (
      !(
        ...
        pastedData.appVersion.startsWith('g-asset-forge-editor') &&
        pastedData.data
      )
    ) {
      return;
    ...
    if (point) {
      ...
      if (dx || dy) {
        GAssetForgeGraphics.dMove(selectedItems, dx, dy);
      }
    }
