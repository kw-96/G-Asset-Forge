import { cloneDeep, parseHexToRGBA, throttle } from '@g-asset-forge/common';
import { getRotatedRectByTwoPoint, isPointEqual } from '@g-asset-forge/geo';
    ...
    for (let i = 0; i < pathData.length; i++) {
      ...
      for (let j = 0; j < pathItem.segs.length; j++) {
        ...
        const anchorControlHandle = new ControlHandle({
          ...
          graphics: new GAssetForgeEllipse(
            {
              objectName: 'anchor',
              width: anchorSize,
              height: anchorSize,
        ...
        const handles = [
          GAssetForgePath.getHandleIn(seg),
          GAssetForgePath.getHandleOut(seg),
        ];
        for (let handleIdx = 0; handleIdx < handles.length; handleIdx++) {
          ...
          const handlePoint = new ControlHandle({
            ...
            graphics: new GAssetForgeRegularPolygon(
              {
